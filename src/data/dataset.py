...
class Dataset(StateDictMixin, TorchDataset):
    ...
    def __getitem__(self, segment_id: SegmentId) -> Segment:
        ...
        segment.info["full_res"] = T.resize(segment.info["full_res"], (256, 256), interpolation=T.InterpolationMode.BICUBIC)
        return segment
    ...
    def load_from_default_path(self) -> None:
        if self._default_path.is_file():    # cwd: '/mnt/disk_3/guanxing/diamond/outputs/2025-01-12/23-29-26'
...
class RobocasaHdf5Dataset(StateDictMixin, TorchDataset):
    def __init__(self, directory: Path) -> None:
        super().__init__()
        # filenames = sorted(Path(directory).rglob("*demo_im512.hdf5"), key=lambda x: int(x.stem.split("_")[-1]))
        # self._filenames = {f"{x.parent.name}/{x.name}": x for x in filenames}
        self.filename = Path(directory) / "demo_im512.hdf5"
        with h5py.File(self.filename, "r") as f:
            # self._filenames = list(f["data/"].keys())
            self._filenames = {k: k for k in list(f["data/"].keys())}
        self._length_one_episode = 100
        self.num_episodes = len(self._filenames)
        self.num_steps = self._length_one_episode * self.num_episodes
        self.lengths = np.array([self._length_one_episode] * self.num_episodes, dtype=np.int64)
    ...
    def __getitem__(self, segment_id: SegmentId) -> Segment:
        ...
        with h5py.File(self.filename, "r") as f:
            # ep = self._filenames[segment_id.episode_id]
            ep = segment_id.episode_id
            images = f[f"data/{ep}/obs/{camera_name}"][()]
            actions = f[f"data/{ep}/actions"][()]
            
            obs = torch.stack([torch.tensor(images[i]).permute(2, 0, 1).div(255).mul(2).sub(1) for i in range(start, stop)])
            act = torch.tensor(np.array([actions[i] for i in range(start, stop)]))

        def pad(x):
            right = F.pad(x, [0 for _ in range(2 * x.ndim - 1)] + [pad_len_right]) if pad_len_right > 0 else x
            return F.pad(right, [0 for _ in range(2 * x.ndim - 2)] + [pad_len_left, 0]) if pad_len_left > 0 else right

        obs = pad(obs)  # [length, 3, 512, 512]
        act = pad(act)  # [length, 12]
        rew = torch.zeros(obs.size(0))
        end = torch.zeros(obs.size(0), dtype=torch.uint8)
        trunc = torch.zeros(obs.size(0), dtype=torch.uint8)
        return Segment(obs, act, rew, end, trunc, mask_padding, info={}, id=SegmentId(segment_id.episode_id, start, stop))
